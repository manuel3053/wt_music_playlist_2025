...
It should be noted there are two subprojects: a (pure) *HTML version*, which is structured as a series of separate webpages; and a *RIA version* (#ts())#footnote[For historic reasons, in the project is is referred as just `js`.], which is structured as a single-page webapp. The functionalities are quite the same. For more information about the requirements for each version see @project-breakdown.
